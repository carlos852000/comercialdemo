...
export class ClienteComponent implements OnInit {
  ...
  moduloOpcion = "modarchcliente";
  itemPrincipal = "Archivos";
  dataUsuario: Session;
  ...
  displayedColumns: string[] = ['correlativo','cliente','telefono','accion'];
  ...
  ngAfterContentInit(): void {
    ...
    this.permisosService.validaPermisosForms(this.dataUsuario.idperfil, this.moduloOpcion).subscribe(response => {
      if (response.dato.permiso == 1) {
        ...
        this.listaClientes(this.pagina);
      } else {
  ...
  listaClientes(pagina) {
    ...
    this.clienteService.listarClientes(
      this.form.get('nombrecliente').value,
      numRegistros,
      numPagina
      ).subscribe(response =>{
  ...
  gestionarCliente(datos){
    const dialogRef = this.dialog.open(ClienteRegistroComponent, {
      width: '650px',
      disableClose: true,
      data: datos
    });
  ...
  eliminarCliente(row) {
    ...
    const sub = dialogRef.componentInstance.onSi.subscribe(data => {
      ...
      this.clienteService.eliminaCliente(
        row.idcliente
      ).subscribe(response =>{
